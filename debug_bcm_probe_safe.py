"""
SAFE probe of BCM routine IDs — uses sub-function 0x03 (Request Results)
instead of 0x01 (Start Routine). This asks the BCM "does this routine exist?"
without actually executing anything.

Same ranges as the fast probe, but zero risk of triggering any routine.
"""
import serial
import time
import sys
...
def nrc_meaning(resp):
    if "7F3111" in resp:
        ...
        return "*** ROUTINE EXISTS (results returned) ***"
    elif "NODATA" in resp:
        return "no response"
    return f"other: {resp}"
...
print(f"\nSAFE SCAN — using sub-function 0x03 (Request Results, NOT Start)")
print(f"Scanning {total_ids} routine IDs across {len(ranges)} ranges...")
print("Nothing will be executed. Showing only VALID routines.\n")
...
for range_start, range_end, range_name in ranges:
    ...
    for rid in range(range_start, range_end):
        ...
        # Sub-function 0x03 = Request Routine Results (does NOT execute)
        hex_cmd = f"3103{rid:04X}"
